...
export class ImgurService {
  ...
  getAllImages(page: number): Observable<Image[]> {
    return this.http.get<any>(API + 'account/anh4bi/images/0.json?perPage=8&page=' + page, httpOptions).pipe(delay(50),
      map(obs => obs.data));
  }

  getImagesCount(): Observable<any> {
    return this.http.get<any>(API + 'account/anh4bi/images/count', httpOptions).pipe(
      delay(50),
      map(obs => obs.data)
    );
  }
  ...
  getAlbums(): Observable<IAlbum[]> {
    return this.http.get<any>(API + 'account/anh4bi/albums', httpOptions).pipe(map(obs => obs.data),
    delay(50));
  }
